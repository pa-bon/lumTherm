...
Luminescent thermometry is one of the possibilities for optical redout of the temperature. It works by exploring T-dependent emission spectra of luminescent species. In particular, a thermometric parameter monitoring a change in ratio of emission intensities in two different parts of the spectrum is often used (Fig. 1). This software aims to facilitate finding the optimal wavelengths, for which the ratio should be calculated to achieve the highest sensitivity.  
**lumTherm** is based on a brute-force approach in which the ratios between all pairs of wavelengths are calculated for all temperatures. Then, the graphical interface based on sensitivity heatmaps will help the user in finding the optimal ones. 
...
The imported data, consiting of a set of spectra measured at different temperatures, are stored as a Pandas `DataFrame`, with the x-axis (`index`) storing wavelengths and the y-axis (`columns`) storing the temperatures.

The analyzed data (the calculated ratios and sensitivities) are stored as 3D NumPy arrays with `x` and `y` corresponding to wavelengths and `z` to temperatures. To reduce memory usage, the numbers are stored as `float32`

The ratios are obtained with simple devision of the intensity at first wavelenght by the intensity at second wavelenght. It is implemented as taking a true matrix product between a column from the imported dataset with the data from that column inversed, then transposed.
...
The adjustable parameters are grouped into sections:
...
2. **Table** - displays the dataframe with data as currently loaded by `pd.read_csv()`. If the import fails, each line of the file is displayed as a new row with a single cell (one column).
...
This tab displays the emission spectra as line plots, with the line color determined by the temperature and follows a gradient (Fig. 3).
...
In the 'examples' directory, there are two files.

The first one (Example1.csv) is used through out the manual, it is the data from Fig. 1 (DOI: 0.26434/chemrxiv-2025-70v85). The correct import parametrs are visible in Fig. 2.
